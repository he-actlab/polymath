...
from polymath.codegen.dnnweavergen.dnnweaver2.scalar.dtypes import Dtype, FQDtype, FixedPoint, Log, Binary, Float, CustomFloat
from polymath.codegen.dnnweavergen.dnnweaver2.utils.utils import lookup_pandas_dataframe
...
class Graph(object):
    ...
    def tensor(self, shape, name=None, dtype=None, trainable=True, data=None):
        ...
        assert name not in self.tensor_registry.keys(), 'Tensor with name {} already exists!'.format(name)
        ...
        try:
            ...
            if self.stack:
                if self.stack[-1] is not default:
                    raise AssertionError('Error in nesting graph_name stacks')
                else:
