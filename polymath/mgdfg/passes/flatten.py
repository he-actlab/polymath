from polymath.mgdfg.serialization import mgdfg_utils
from polymath.mgdfg.serialization.mgdfg_pb2 import Component, ValueInfo
import logging
from polymath.mgdfg.passes.pass_utils import is_literal, is_number
...
def flatten_graph(output_graph: Component, graph, templates, context,edge_node_ids, arg_map):
    ...
    for e in graph.edge_info:
        ...
        if is_literal(e):
            ...
            copy_edge.attributes['alias'].CopyFrom(mgdfg_utils.make_attribute('alias', arg_map[e].name))
        else:
            uid = context + e
            copy_edge.CopyFrom(graph.edge_info[e])
            copy_edge.name = uid
        ...
        if graph.name != 'main':
            ordered_args = mgdfg_utils.get_attribute_value(graph.attributes['ordered_args'])
        else:
            ordered_args = []

        if 'dimensions' in list(copy_edge.attributes):
            dims = mgdfg_utils.get_attribute_value(copy_edge.attributes['dimensions'])
            new_dims = []
            for d in dims:
                if d in arg_map.keys():
                    ...
                    new_dims.append(d)
            copy_edge.attributes['dimensions'].CopyFrom(mgdfg_utils.make_attribute('dimensions', new_dims))
        ...
        if uid not in edge_node_ids['edges'].keys():
            ...
            if e not in arg_map.keys():
                ...
                output_graph.edge_info[uid].attributes['component_type'].CopyFrom(
                    mgdfg_utils.make_attribute('component_type', graph.op_type))
    ...
    for n in graph.sub_graph:
        ...
        if op_cat == 'component':
            ...
            instance_args = mgdfg_utils.get_attribute_value(n.attributes['ordered_args'])
            ctemplate = templates[n.op_type]
            signature_args = mgdfg_utils.get_attribute_value(ctemplate.attributes['ordered_args'])
...
def create_map(instance_args, signature_args, instance_edges, signature_edges, op=None):
    ...
    for i in range(len(instance_args)):
        ...
        idims = mgdfg_utils.get_attribute_value(instance_edges[iarg].attributes['dimensions'])
        iid_literal = False
        if instance_edges[iarg].iid:
            inst_iid = instance_edges[iarg].iid
            iid_literal = is_literal(inst_iid)

        sdims = mgdfg_utils.get_attribute_value(signature_edges[sarg].attributes['dimensions'])
        ...
        if len(idims) != len(sdims) and not iid_literal:
            ...
            for d in range(len(idims)):
                ...
                carg_map[sig_dim].attributes['vtype'].CopyFrom(mgdfg_utils.make_attribute('vtype', 'scalar'))
    ...
    if len(signature_args) > len(instance_args):
        ...
        for default in signature_args[start:]:
            ...
            if 'default' not in sig_attr:
                ...
                def_val = mgdfg_utils.get_attribute_value(signature_edges[default].attributes['default'])
                carg_map[default] = signature_edges[default]
                carg_map[default].attributes['value'].CopyFrom(mgdfg_utils.make_attribute('value', def_val))
                if is_number(def_val):
                    def_val = str(def_val)
                carg_map[default].name = def_val
                carg_map[default].attributes['vtype'].CopyFrom(mgdfg_utils.make_attribute('vtype', 'scalar'))
    for e in op.edge_info:
        vcat = mgdfg_utils.get_attribute_value(op.edge_info[e].attributes['vcat'])
        if vcat == 'declaration':
            dims = mgdfg_utils.get_attribute_value(op.edge_info[e].attributes['dimensions'])
